...
pub use pallet::*;
use pallet_tds_ipfs_core::storage::{set_offchain_data, OffchainStorageData};
...
pub mod pallet {
	...
	use pallet_tds_ipfs_core::storage::set_offchain_storage_data_for_key;
	use sp_std::{str, vec::Vec};

	use pallet_tds_ipfs_core::{
		addresses_to_utf8_safe_bytes, generate_id, ipfs_request, ocw_parse_ipfs_response,
		ocw_process_command, storage::set_offchain_data, CommandRequest, Error as IpfsError,
	...
	**/
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			let key = b"key-test-storage-3".to_vec();
			let value = pallet_tds_ipfs_core::storage::offchain_storage_raw_data_for_key(&key);

			if let Some(data) = value {
				log::info!("got offchain data with: {:?}", data);
			}
		}
	}
	...
	impl<T: Config> Pallet<T> {
		pub fn test() {
			...
			let received_bytes = b"value".to_vec();

			let obj = pallet_tds_ipfs_core::storage::OffchainStorageData::new(received_bytes);
			pallet_tds_ipfs_core::storage::set_offchain_storage_data_for_key::<T>(
				&key, &obj, false,
			);
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_bytes(
			...
			set_offchain_storage_data_for_key::<T>(
				&b"test-key-1".to_vec(),
				&OffchainStorageData::new(b"test-value-1".to_vec()),
				false,
			);
